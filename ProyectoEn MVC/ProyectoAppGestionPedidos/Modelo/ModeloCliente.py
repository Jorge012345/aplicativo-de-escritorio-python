...
class Clientes(Frame):
	...
	def __init__(self,raiz):
		self.raiz=raiz
		#creando contenedor
		frame=LabelFrame(text="Registra un nuevo cliente",bg='white')
		...
		self.subTitulo=Label(frame,text='Registro de Cliente',fg='red',bg='white')
		self.subTitulo.grid(row=5,column=0,columnspan=6)
		#creando tabla
		col = ('Usuario', 'Contraseña','DNI', 'Telefono')
		self.tree=ttk.Treeview(frame,height=4,column=col)
		...
		#style.configure('Treeview',background='#42B067',foreground='black',rowheight=50,fieldbackground='silver')
		#style.map('Treeview',background[('selected','#42B067')])
		#botones 
	...
	def eliminar_cliente(self):	
		...
		query='DELETE FROM Cliente WHERE Usuario=?'
	...
	def editar_cliente(self):
		...
		try:
			...
			self.message['text']='Selecciona un cliente'
			return
		...
		self.editar_ventana.title('Editar cliente')
		# codigo antiguo
		...
		#precio nuevo
		Label(self.editar_ventana,text='DNI nuevo: ').grid(row=1,column=2)
	...
	def editar_fila(self,nuevo_usuario,antiguo_usuario,nuevo_contrasenia,antiguo_contrasenia,nuevo_dni,antiguo_dni,nuevo_telefono,antiguo_telefono):
		query='UPDATE Cliente SET Usuario=?,Contraseña=?,Dni=?,Telefono=? WHERE Usuario=? AND Contraseña=? AND Dni=? AND Telefono=?' 
